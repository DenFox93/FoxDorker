...
def token_round_robin():
    ...
    #print("token used --> "+ str(n) + " " + str(current_token) + "\n")
    #print("token n. " +str(n))
...
def api_search(url):
        ...
        try:
            ...
            r = requests.get(url, headers=headers, timeout=10)
            print("HTTP error occured, but resolved")
        json = r.json()
        ...
        if('total_count' in json):
            ...
            if ((url_results_dict[url] > 0) and (len(query) < 128)):
                ...
                for x in range(length):
                    ...
                    block=["helper","CHANGELOG","README",".md",".png","example","mock","test","sample","default","translation","translate","language","Localiz","schema","template"]
                    ...
                    try:
                        ...
                        response = urllib.request.urlopen(z)
                        print("urllib HTTP error in z captured!!")
                    data = response.read()
...
print(colored("[-] error occurred udentified: %s" % error, 'red'))
